	...
	while (true)
	{
		switch (viTri)
		{
			case 0:
			{
				...
				if (flag == 1)
				{
					int stt = timKiemDSTheoMa(l, s.ISBN);
					if (stt == -1)
					{
						xoaThongBao();
...
void inMotDS(dausach* s, int position)
{
	RemoveOldData( sizeof(thongTinDS) / sizeof(string), position);// xoa dong noi dung cu truoc do
